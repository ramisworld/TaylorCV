...
assert.equal(ai.colourSystem.accentPalette, "graphite");
assert.equal(aiTokens.bodyTextColor, "#111827");
assert.equal(aiTokens.mutedTextColor, "#374151");
...
const qualityWarnings = collectCvQualityWarnings({
  ...
  jobContext: {
    ...
    responsibilities: ["Build product-facing AI systems"],
    proofNeeds: ["Shipped systems", "projects"],
    keywords: ["AI", "deployment", "evaluation"],
    risks: [],
    exactPhrases: ["AI systems", "evaluation"],
  },
  sectionSignals: {
    candidateEvidenceLevel: "credible",
    candidatePresentationStage: "graduate",
    ...
    founderFramingGuidance: "Prefer engineering framing for employee applications.",
    recommendedFocus: "Lead with shipped AI project proof.",
    positioningWarnings: [],
  },
  cv: warningCv,
});
